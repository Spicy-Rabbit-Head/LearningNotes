...
| 函数                  | 描述                               |
| --------------------- | ---------------------------------- |
| [GET_BIT](#GET_BIT)   | 从二进制字符串中提取指定的位并返回 |
| [GET_BYTE](#GET_BYTE) | 从二进制字符串中提取指定的字节     |
| [SET_BIT](#SET_BIT)   | 将二进制字符串中的指定为更新       |
| [SET_BYTE](#SET_BYTE) | 将二进制字符串中的指定字节更新     |
| [SHA224](#SHA224)     | 计算二进制字符串的 SHA-224 哈希    |
| [SHA256](#SHA256)     | 计算二进制字符串的 SHA-256 哈希    |
| [SHA384](#SHA384)     | 计算二进制字符串的 SHA-384 哈希    |
| [SHA512](#SHA512)     | 计算二进制字符串的 SHA-512 哈希    |
...
| 函数                                  | 描述                                            |
| ------------------------------------- | ----------------------------------------------- |
| [ABBREV](#ABBREV)                     | 创建文本形式的缩写显示格式                      |
| [BROADCAST](#BROADCAST)               | 计算地址网络的广播地址                          |
| [FAMILY](#FAMILY)                     | 返回地址的族                                    |
| [HOST](#HOST)                         | 以文本形式返回 IP 地址,忽略网络掩码             |
| [HOSTMASK](#HOSTMASK)                 | 计算地址网络的主机掩码                          |
| [INET_MERGE](#INET_MERGE)             | 计算包含两个给定网络的最小网络                  |
| [INET_SAME_FAMILY](#INET_SAME_FAMILY) | 测试地址是否属于同一 IP 系列                    |
| [MASKLEN](#MASKLEN)                   | 返回网络掩码长度（以位为单位）                  |
| [NETMASK](#NETMASK)                   | 计算地址网络的网络掩码                          |
| [NETWORK](#NETWORK)                   | 返回地址的网络部分,将网络掩码右侧的任何内容清零 |
| [SET_MASKLEN](#SET_MASKLEN)           | 设置值的网络掩码长度 <br />地址部分不会更改     |
| [TEXT](#TEXT)                         | 以文本形式返回未缩写的 IP 地址和网络掩码长度    |
| [TRUNC](#TRUNC)                       | MAC 地址 将地址的最后 3 个字节设置为零          |
...
| 函数                                            | 描述                                                         |
| ----------------------------------------------- | ------------------------------------------------------------ |
| [ARRAY_TO_TSVECTOR](#ARRAY_TO_TSVECTOR)         | 将文本字符串数组转换为原样用作词素                           |
| [GET_CURRENT_TS_CONFIG](#GET_CURRENT_TS_CONFIG) | 返回当前默认文本搜索配置的 OID                               |
| [NUMNODE](#NUMNODE)                             | 返回词素加运算符数                                           |
| [PLAINTO_TSQUERY](#PLAINTO_TSQUERY)             | 将文本转换为根据指定或默认配置规范化单词                     |
| [WEBSEARCH_TO_TSQUERY](#WEBSEARCH_TO_TSQUERY)   | 将文本转换为根据指定或默认配置规范化单词                     |
| [QUERYTREE](#QUERYTREE)                         | 生成的可索引部分的表示形式 <br />结果为空或仅指示不可索引的查询 |
| [SETWEIGHT](#SETWEIGHT)                         | 为矢量的每个元素分配指定的权重                               |
| [STRIP](#STRIP)                                 | 删除位置和权重                                               |
| [TO_TSVECTOR](#TO_TSVECTOR)                     | 将文本转换为根据指定或默认配置规范化单词                     |
| [JSON_TO_TSVECTOR](#JSON_TO_TSVECTOR)           | 选择 JSON 文档中筛选器请求的每个项目,<br />并将每个项目转换根据指定的或默认配置规范化单词 |
| [TS_DELETE](#TS_DELETE)                         | 从向量中删除给定词素的任何匹配项                             |
| [TS_FILTER](#TS_FILTER)                         | 从向量中选择具有给定权重的元素                               |
| [TS_HEADLINE](#TS_HEADLINE)                     | 以缩写形式显示文档中查询的匹配项,该匹配项必须是原始文本 <br />而不是文档中的单词在与查询匹配之前根据指定的或默认配置进行规范化 |
| [TS_RANK](#TS_RANK)                             | 计算显示向量与查询匹配程度的分数                             |
| [TS_RANK_CD](#TS_RANK_CD)                       | 使用覆盖密度算法计算显示矢量与查询匹配程度的分数             |
| [TS_REWRITE](#TS_REWRITE)                       | 将查询中出现的目标替换为替换                                 |
| [TSQUERY_PHRASE](#TSQUERY_PHRASE)               | 构造一个短语查询 <br />该查询在连续的词素（与运算符相同）中搜索查询 1 和查询 2 的匹配项 |
| [TSVECTOR_TO_ARRAY](#TSVECTOR_TO_ARRAY)         | 转换为词素数组                                               |
| [TS_DEBUG](#TS_DEBUG)                           | 根据指定的或默认的文本搜索配置从文档中提取和规范化令牌       |
| [TS_LEXIZE](#TS_LEXIZE)                         | 如果字典知道输入标记,则返回替换词素数组 <br />如果字典知道该标记但它是一个停用词,则返回一个空数组 <br />如果它不是已知单词,则返回 NULL |
| [TS_PARSE](#TS_PARSE)                           | 使用命名分析器从文档中提取令牌                               |
| [TS_TOKEN_TYPE](#TS_TOKEN_TYPE)                 | 返回一个表,该表描述命名分析程序可以识别的每种类型的标记      |
| [TS_STAT](#TS_STAT)                             | 执行 sqlquery（必须返回单个列）, <br />并返回有关数据中包含的每个不同词法的统计信息 |
...
| 函数                                                        | 描述                                                         |
| ----------------------------------------------------------- | ------------------------------------------------------------ |
| [XMLPARSE](#XMLPARSE)                                       | 将字符串转换为 XML 值                                        |
| [XMLCOMMENT](#XMLCOMMENT)                                   | 创建一个 XML 值,其中包含以指定文本作为内容的 XML 注释        |
| [XMLCONCAT](#XMLCONCAT)                                     | 连接单个 XML 值的列表,以创建包含 XML 内容片段的单个值        |
| [XMLELEMENT](#XMLELEMENT)                                   | 生成具有给定名称、属性和内容的 XML 元素                      |
| [XMLFOREST](#XMLFOREST)                                     | 使用给定的名称和内容生成元素的 XML（序列）                   |
| [XMLPI](#XMLPI)                                             | 创建一个 XML 处理指令                                        |
| [XMLROOT](#XMLROOT)                                         | 更改 XML 值的根节点的属性                                    |
| [XML_IS_WELL_FORMED](#XML_IS_WELL_FORMED)                   | 检查字符串是否表示格式正确的 XML,并返回布尔结果              |
| [XML_IS_WELL_FORMED_DOCUMENT](#XML_IS_WELL_FORMED_DOCUMENT) | 检查字符串是否表示格式正确的 XML 文档,并返回布尔结果         |
| [XML_IS_WELL_FORMED_CONTENT](#XML_IS_WELL_FORMED_CONTENT)   | 检查字符串是否表示格式正确的 XML 内容片段,并返回布尔结果     |
| [XPATH](#XPATH)                                             | 根据 XML 值 xml 计算 XPath 1.0 表达式 xpath（以文本形式给出） |
| [XPATH_EXISTS](#XPATH_EXISTS)                               | 指示查询是否满足                                             |
| [XMLTABLE](#XMLTABLE)                                       | 生成基于 XML 值的表、用于提取行的 XPath 筛选器和一组列定义   |
| [TABLE_TO_XML](#TABLE_TO_XML)                               | 将表映射到 XML 值                                            |
| [QUERY_TO_XML](#QUERY_TO_XML)                               | 将查询映射到 XML 值                                          |
| [CURSOR_TO_XML](#CURSOR_TO_XML)                             | 将游标映射到 XML 值                                          |
